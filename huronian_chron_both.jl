# Huronian_chron - bayesian age depth modelling for the Huronian detrital
# zircons in the Medicine Bow Mountains and the Sierra Madre region, WY
# Cox and Keller, 2022-02-25
...
pathtodata = "./sm"
samplenames = replace.(filter(x->occursin(r"[0-9].*csv", x), readdir(pathtodata)),".csv"=>"");
nSamples = length(samplenames) # The number of samples you have data for
smpl = NewChronAgeData(nSamples)
smpl.Name      =  (samplenames...,)
smpl.Height   .=  1.:nSamples
smpl.Height_sigma .= 0.0
smpl.Age_Sidedness .= zeros(nSamples) # Sidedness (zeros by default: geochron constraints are two-sided). Use -1 for a maximum age and +1 for a minimum age, 0 for two-sided
smpl.Path = pathtodata # Where are the data files?
...
distSteps = 1*10^5 # Number of steps to run in distribution MCMC
